...
const mount = async (config: BackendConfig, path: string) => {
	...
	if (os.platform() !== "linux") {
		const message = isDarwin()
			? "WebDAV volume mounting is not supported on macOS. Use the Directory backend or a cloud repository instead."
			: "WebDAV mounting is only supported on Linux hosts.";
